...
def visualize_tidal_data(pkl_file_name):
    ...
    ax1.set_xticklabels([str(x) if x % 200 == 0 else '' for x in grid_ticks])
    ax1.set_yticklabels([str(y) if y % 200 == 0 else '' for y in grid_ticks])
    ...
    # 绘制方块函数
    def draw_grid_block(cx, cy, h, edge_color, line_width, line_style='-'):
        ...
        ax1.text(cx, cy, f"{int(h)}m", color=text_color, ha='center', va='center', fontsize=9, fontweight='bold', zorder=3)

    # 1. 绘制无需求遮挡物 (灰色虚线边框)
    if len(obs_coords) > 0:
        for (x, y), h in zip(obs_coords, obs_heights):
            draw_grid_block(x, y, h, 'dimgray', 2, '--')

    # 2. 绘制住宅区 (明绿色实线边框)
    R_mask = node_types == 0
    for (x, y), h in zip(coords[R_mask], heights[R_mask]):
        draw_grid_block(x, y, h, 'limegreen', 3)

    # 3. 绘制商业区 (红色实线边框)
    C_mask = node_types == 1
    for (x, y), h in zip(coords[C_mask], heights[C_mask]):
        draw_grid_block(x, y, h, 'red', 3)

    ax1.set_title("3D 城市建筑网格 (颜色越深高度越高)", fontsize=14, fontweight='bold')
    ...
    ax1.legend(handles=legend_elements, loc='upper right', bbox_to_anchor=(1.05, 1.15))

    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
    cbar = fig.colorbar(sm, ax=ax1, fraction=0.046, pad=0.04)
    cbar.set_label('建筑 Z 轴高度 (米)', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(data_dir, pkl_file_name.replace('.pkl', '_UnifiedGrid_Heights.png')), dpi=300, bbox_inches='tight')
    plt.show() 
